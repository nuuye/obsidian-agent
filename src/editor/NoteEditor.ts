...
export class NoteEditor {
    ...
    private addAliases(content: string, topics: string[]): string {
        ...
        // Avoid aliases generation if we detect ---
        if (/^---\r?\n/.test(content)) {
            return content;
        }

        const aliasLines = topics.map((topic) => `  - "${topic.replace(/"/g, '\\"')}"`).join("\n");
    ...
    async edit(
        ...
        6. ${
            analysis.schema.useful && analysis.schema.score > 0.6
